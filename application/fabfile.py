...
def development():
	...
	env.config_dir = 'conf/' + inspect.getframeinfo(inspect.currentframe()).function
...
def deploy():	
	...
	sudo('mkdir -p %s/%s/conf' % (env.deploy_base_dir, env.to_deploy))
	...
	upload_dir('%s' % env.config_dir, '%s/%s/conf' % (env.deploy_base_dir, env.to_deploy))
